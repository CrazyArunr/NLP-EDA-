...
st.image('friends.jpg')


st.write('Welcome to Friends EDA analysis')
...
mean_col = df.groupby(by='Year_of_prod')["Stars"].mean()
df = df.set_index(['Year_of_prod'])
df['mean_col'] = mean_col
df = df.reset_index() 
df.head()

mean_vote = df.groupby(by ='Season')["Votes"].mean() 
...
trigrams_df = pd.DataFrame(trigrams_tf.todense(), columns=tf_trigrams.get_feature_names_out())
trigrams_df.sum().sort_values(ascending=False).head(30)

top_texts3 = trigrams_df.sum().sort_values(ascending=False)
top_texts3.head(15).plot(kind='barh')
...
trigrams_df1 = pd.DataFrame(trigrams_tf1.todense(), columns=tf_trigrams1.get_feature_names_out())
